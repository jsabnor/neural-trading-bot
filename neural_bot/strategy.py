...
@tf.keras.utils.register_keras_serializable(package='neural_bot')
class AttentionLayer(layers.Layer):
    ...
    def call(self, inputs):
        ...
        score = tf.nn.tanh(tf.matmul(inputs, self.W) + self.b)  # (batch, timesteps, units)
        # Reduce to single score per timestep
        score = tf.reduce_mean(score, axis=-1, keepdims=True)  # (batch, timesteps, 1)
        # Normalize scores across timesteps
...
class BuyMetricsCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(f"\n📊 Epoch {epoch+1}: Recall BUY={recall_buy:.3f}, F1 BUY={f1_buy:.3f}")
...
class FocalLoss(tf.keras.losses.Loss):
    ...
    Penaliza más los errores en clases minoritarias (SELL, HOLD)
...
class FeatureExtractor:
    ...
    def calculate_technical_indicators(self, df):
        ...
        for name, period in config.TECHNICAL_INDICATORS.items():
            if 'ema' in name and name in ['ema_fast', 'ema_slow', 'ema_trend']:
                ...
                # Calcula el ángulo de la tendencia (en radianes o simplemente delta)
    ...
    def calculate_market_regime(self, df):
        """
        Calcula features de régimen de mercado para mejor adaptación a condiciones
        
        Returns:
            DataFrame con features de régimen añadidas
        """
        result = df.copy()
        ...
        if 'atr' in result.columns:
            ...
            # Normalizar usando rolling min/max (50 períodos)
    ...
    def extract_features(self, df, fit_scaler=False):
        ...
        if fit_scaler:
            ...
            # CRÍTICO: Clip valores fuera del rango de entrenamiento
            X = np.clip(X, 0, 1)
...
class NeuralTradingModel:
    ...
    def build_model(self):
        ...
        if config.USE_ATTENTION:
            ...
            # Attention Mechanism (usando AttentionLayer para serialización correcta)
            x = AttentionLayer(units=config.ATTENTION_UNITS)(lstm_out)
        else:
        ...
        if config.USE_FOCAL_LOSS:
            ...
            loss_name = f"Focal Loss (γ={config.FOCAL_LOSS_GAMMA}, α={config.FOCAL_LOSS_ALPHA})"
        else:
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, epochs=None, class_weights=None):
        ...
        if class_weights is None:
            ...
            print(f"\n⚖️ Class Weights (desde config):")
        else:
            class_weight_dict = class_weights
            print(f"\n⚖️ Class Weights (calculados automáticamente):")
        ...
        # Añadir tu callback personalizado a la lista existente
    ...
    def evaluate(self, X_test, y_test):
        ...
        # Definir nombres de clases según modo
...
class ContinuousLearner:
    ...
    def prepare_training_data(self, symbols=None, timeframe='4h', start_date=None, end_date=None):
        ...
        for symbol in symbols:
            ...
            if config.FILTER_LATERAL_MARKETS:
                print(f"    🔍 Filtrando mercados laterales (ADX < {config.MIN_ADX_FOR_TRAINING})...")
                original_len = len(df)
                ...
                # Separar datos por régimen de mercado
                ...
                print(f"    📊 Filtrado: {len(df_trend)} tendencias + {len(df_lateral)} laterales = {len(df)} velas")
                print(f"    🗑️ Eliminadas: {removed} velas laterales ({removed/original_len*100:.1f}%)")
            ...
            print(f"   🔍 Features extraídas: {self.feature_extractor.feature_names}")
        ...
        #     print(f"\n📊 Oversampling aplicado: {len(idx_buy)} BUY duplicados → {len(X_train)} muestras finales")


        # ============================================================================
        # UNDERSAMPLING FÍSICO DESACTIVADO
        # ============================================================================
        # RAZÓN: Evitar triple penalización (Undersampling + Class Weights + Focal Loss)
        # Ahora usamos solo AUTO_CLASS_WEIGHTS que balancea automáticamente.
        # 
        # El código original reducía agresivamente HOLD, pero esto causaba:
        # 1. Confusión en el modelo por señales contradictorias
        # 2. Pérdida de datos valiosos de mercados laterales
        # 3. Overfitting a movimientos extremos
        # ============================================================================
        
        # CÓDIGO ORIGINAL COMENTADO:
        # # 5. UNDERSAMPLING FÍSICO (Para eliminar bias de HOLD)
        # print("\n⚖️ Aplicando UNDERSAMPLING físico...")
        # 
        ...
        # 
        # # Determinar tamaño objetivo (el de la clase minoritaria, o un poco más)
        ...
        #     print(f"   ✂️ Recortando HOLD de {n_hold} a ~{target_size}...")
        ...
        #     print(f"   Distribución: SELL={n_sell}, HOLD={target_size}, BUY={n_buy}")
        # else:
        #     print("   ✅ No se requiere undersampling agresivo.")
        
        # Mostrar distribución actual (sin undersampling)
        from collections import Counter
        class_counts = Counter(y_train)
        print("\n📊 Distribución de clases (SIN undersampling físico):")
        ...
        print(f"   ℹ️ El balanceo se hará mediante CLASS_WEIGHTS durante entrenamiento")
        ...
        if config.USE_AUTO_CLASS_WEIGHTS:
            ...
            print("\n⚖️ Pesos de clases calculados AUTOMÁTICAMENTE (Balanced):")
        else:
            # Opción B: Manual (desde config)
            class_weights_dict = config.CLASS_WEIGHTS
            print("\n⚖️ Pesos de clases MANUALES (desde config):")
        ...
        for cls, weight in class_weights_dict.items():
            ...
            print(f"   {label} ({cls}): {count} muestras ({percentage:.1f}%) → Peso: {weight:.3f}")
        ...
        print(f"   ⚠️ IMPORTANTE: BUY debe tener peso ALTO para ser aprendido correctamente")
    ...
    def train_initial_model(self, symbols=None, timeframe='4h', start_date=None, end_date=None, epochs=None):
        ...
        Args:
            ...
            timeframe: Timeframe
            start_date: Fecha inicio (YYYY-MM-DD) o None para todo el histórico
...
class NeuralStrategy:
    ...
    def __init__(self, version=None, model_name=None):
        """
        Args:
            version: Versión del modelo a cargar (sistema legacy, None = última)
    ...
    def load_model_by_name(self, name=None):
        ...
        if result is None:
            print(f"❌ No se pudo cargar modelo '{name}'")
            return False
    ...
    def predict_signal(self, X):
        ...
        # NUEVO: Filtro anti-confusión BUY/SELL
        # Si predice BUY pero SELL tiene probabilidad alta, es señal ambigua → HOLD
        # Si predice SELL pero BUY tiene probabilidad alta, es señal ambigua → HOLD
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        # Si se especificó un nombre, guardar con ModelManager
        if args.name and model is not None:
            ...
            if manager.save_model(model.model, learner.feature_extractor.scaler, args.name, metadata):
                ...
                print(f"\n⚠️ Error guardando modelo con nombre: {args.name}")
